...
class SubstitutionTest extends AnyFlatSpec with should.Matchers {
  ...
  "substitute x by CInt(5) in App(inc, Id(x))" should "return App(inc, CInt(5))" in { //teste que pega o erro na aplicacao de funcao do substitute. erro feito pelo professor
    val c5 = CInt(5)
    val app = App("inc", Id("x"))
    substitute(c5, "x", app) should be (App("inc", CInt(5)))
  }
